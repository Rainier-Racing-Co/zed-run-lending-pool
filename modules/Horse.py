from modules.query_summary_horse_data import get_horse_data
...
class Horse:
    def __init__(self, horse_obj):
        ...
        self.mother = [] if len(horse_obj['data.horse.parents'][0]) < 1 else horse_obj['data.horse.parents'][0][0]
        self.father = [] if len(horse_obj['data.horse.parents'][0]) < 1 else horse_obj['data.horse.parents'][0][1]
...
if __name__ == '__main__':
    horse_obj = get_summary_horse_data(379942)
    horse_class = Horse(horse_obj)
    print(horse_class.name)
